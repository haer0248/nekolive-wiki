...
如：斗內通知、斗內列表、進度條  
這些都是因為 **「從平台建立訂單而可以自動更新的數值」**，當然要選擇不使用也可以，從第三方的來源因為並不是由我們的伺服器發送的訂單請求，所以我們是接收不到的，但是可以前往「側邊欄」中的「建立通知」來進行模擬通知或建立站外訂單。
...
:::tip 
金額區間套用於「OBS 通知圖片及音效」及「斗內卡片」
:::

0 (即1) 元 ~ 50 元會套用 1，也就是 50 以下，
50 以上就會套用 2，也就是 50~300，
300 以上就會套用 3，也就是 300~1000，
1000 以上就會套用 4。

![Image](/images/faq/money-range.png)